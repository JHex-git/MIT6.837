...
RayTracer::RayTracer(std::shared_ptr<SceneParser> s, int max_bounces, float cutoff_weight, 
                        ...
                        m_shadows(shadows), m_shade_back(shade_back), m_use_grid(grid), m_visualize_grid(visualize_grid)
{
    if (m_use_grid)
    {
        assert(nx > 0 && ny > 0 && nz > 0);
        m_grid = std::make_shared<Grid>(m_scene_parser->getGroup()->getBoundingBox(), nx, ny, nz, m_visualize_grid);
...
{
    if (m_visualize_grid && m_use_grid) return traceRayGrid(ray, tmin, bounces, weight, indexOfRefraction, hit);
    else if (m_use_grid) return traceRaySceneFast(ray, tmin, bounces, weight, indexOfRefraction, hit);
...
Vec3f RayTracer::traceRaySceneFast(const Ray &ray, float tmin, int bounces, float weight, 
                    float indexOfRefraction, Hit &hit) const
{
    // Reinitialize hit to be a miss.
    hit.set(tmin, nullptr, Vec3f(0, 0, 0), ray);
    ...
    if (m_grid->intersect(ray, hit, tmin))
    {
        ...
        if (reflective_color != Vec3f(0, 0, 0))
        {
            Ray reflect_ray(ray.pointAtParameter(hit.getT()), mirrorDirection(normal, ray.getDirection()));
            Hit reflect_hit;
            // color += reflective_color * traceRaySceneFast(reflect_ray, tmin, bounces + 1, weight * reflective_color.Length(), indexOfRefraction, reflect_hit);
            color += reflective_color * traceRaySceneFast(reflect_ray, epsilon, bounces + 1, weight * reflective_color.Length() / std::sqrt(3), indexOfRefraction, reflect_hit);
            if (reflect_hit.getT() != 0)
            {
                RayTree::AddReflectedSegment(reflect_ray, 0, reflect_hit.getT());
            }
        ...
        if (transparent_color != Vec3f(0, 0, 0))
        {
            ...
            if (has_transmitted)
            {
                Ray transmitted_ray(ray.pointAtParameter(hit.getT()), transmitted_direction);
                Hit transmitted_hit;
                // transmitted_color = traceRaySceneFast(transmitted_ray, tmin, bounces + 1, weight * transparent_color.Length(), hit.getMaterial()->getIndexOfRefraction(), transmitted_hit);
                color += transparent_color * traceRaySceneFast(transmitted_ray, epsilon, bounces + 1, weight * transparent_color.Length() / std::sqrt(3), hit.getMaterial()->getIndexOfRefraction(), transmitted_hit);
                if (transmitted_hit.getT() != 0)
                {
                    RayTree::AddTransmittedSegment(transmitted_ray, 0, transmitted_hit.getT());
                }
